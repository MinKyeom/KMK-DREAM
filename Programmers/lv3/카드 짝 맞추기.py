        ...
        for nx, ny in zip(dx, dy):
            ...
            while True:
                ...
                if new_x > 3 or new_x < 0 or new_y > 3 or new_y < 0:
                    ...
                    if not (new_x, new_y) in visit and flag == False:
                        q.append((new_x, new_y, k + 1))

                        if fir[new_x][new_y] == 1:
                            break

                        flag = True

                    # ctrl를 눌렀을 때 고려
                    elif not (new_x, new_y) in visit and fir[new_x][new_y] == 1:
                        q.append((new_x, new_y, k + 1))
                        break
...
def count(k, distance):
    ...
    while q:
        ...
        q.append([order_one, fir, [sec_x, sec_y], count_one + 2])

        # location > two > one
        two = start(new_x, new_y, sec_x, sec_y, count, sec)
        count_two += two
        two = start(sec_x, sec_y, fir_x, fir_y, count, sec)
        count_two += two
        ...
        q.append([order_two, sec, [fir_x, fir_y], count_two + 2])
