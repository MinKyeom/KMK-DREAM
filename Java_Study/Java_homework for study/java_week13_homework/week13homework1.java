...
public class week13homework1 {
	public static void main(String[] args) {
		ArrayList box = new ArrayList();
		box.add("전체 박스");
		box.add(new Box5("white",3,3,3));
		box.add(new Box5("red",5,5,5));
		box.add(new Box5("black",7,7,7));		
		System.out.println(box.get(0) + " : " + box.subList(1,4));
		for(int i = 1 ; i < box.size() ; i++) {
			Box5 b = (Box5)box.get(i);
...
class Box5 {
	...
	public Box5(String c, int w, int d, int h) {
		color = c;
		vol = w*d*h;
	}
