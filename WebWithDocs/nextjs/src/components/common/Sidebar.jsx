  ...
  return (
    <>
      ...
      <div className={`sidebar-menu ${isSidebarOpen ? "open" : ""}`}>
        ...
        >
          <span className="logo-text-header" style={{ fontSize: "1.8rem" }}>
            MinKowski
          </span>
        ...
        <div className="sidebar-content" style={{ flex: 1, overflowY: "auto" }}>
          {/* 사이드바 내부 인증 섹션 */}
          <div
            className="sidebar-auth-section"
          ...
          >
            {/* 영문 변경 및 필기체 스타일 클래스 추가, 한 줄씩 배치 */}
            <Link href="/" onClick={closeSidebar} className="cursive-item">
              Home
            </Link>
            <Link href="/post" onClick={closeSidebar} className="cursive-item">
              All Posts
            </Link>
          </div>
          ...
          <div className="sidebar-nav-list" style={{ marginTop: "10px" }}>
            {isLoading ? (
              ...
              categories.map((cat) => (
                <Link
                  key={cat.name}
                  href={`/post?category=${cat.slug || cat.name}`}
                  onClick={closeSidebar}
                >
