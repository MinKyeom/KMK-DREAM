...
export const metadata = {
    title: '전체 포스트 목록',
    description: 'Dev Blog의 모든 개발 포스트 목록입니다. 원하는 글을 찾아보세요.',
...
export default async function PostListPage({ searchParams }) {
  ...
  return (
    <div className="homepage-container">
      <h1 className="section-title" style={{ marginTop: '30px', marginBottom: '40px', fontSize: '2.5rem' }}>
      ...
      {posts.length === 0 ? (
        ...
        <div className="post-list" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
      ...
      {pageInfo.totalPages > 1 && (
        <div className="pagination-controls" style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '40px' }}>
            {/* 이전 페이지 버튼 */}
            <Link 
            ...
            <Link
                href={`/post?page=${currentPage + 1}`}
                className="btn-secondary"
                style={{ pointerEvents: currentPage === pageInfo.totalPages - 1 ? 'none' : 'auto', opacity: currentPage === pageInfo.totalPages - 1 ? 0.5 : 1 }}
            >
                다음
            </Link>
        </div>
